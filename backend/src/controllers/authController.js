...
export const sendResetOTP = async (req, res) => {
  try {
    ...
    await transporter.sendMail({
      from: "Cinema App <absolutecinema.noreply@gmail.com>",
      to: email,
      subject: "Mã OTP khôi phục mật khẩu",
      html: `
        <p>Xin chào ${user.full_name},</p>
        <p>Mã OTP khôi phục mật khẩu của bạn là: ${otp}</p>
    ...
    console.error("Lỗi khi chạy sendResetOTP", error);
    return res.status(500).json({ message: "Lỗi hệ thống" });
  }
};
...
export const verifyOTPandReset = async (req, res) => {
  try {
    ...
    if (!user)
      return res.status(404).json({ message: "Email sai hoặc Otp không đúng" });
    ...
    console.error("Lỗi khi chạy verifyOTPandReset", error);
    return res.status(500).json({ message: "Lỗi hệ thống" });
  }
};
